...
This is a simple *Windows Console Application* that allows control your Philips Hue system from the commandline or by using batch-files. Good for testing or scheudle tasks. For continuous usage see [HueHookService](https://github.com/rmmlr/HueHook/blob/master/HueHookService), also contains in this repository.
...
Run the HueHookServer.exe with some start parameters.
...
The program must be started with at least the required parameters, marked with :warning:. In addition, further allowed parameters :heavy_check_mark: can be appended.
...
To obtain this parameters you can start the program with a shortcut. Append the parameters to the target path, e.g. `C:\path-to-program\HueCmd.exe -light.hue id=1 on=1 bri=127`.
